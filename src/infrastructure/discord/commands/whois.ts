...
export async function handleWhoisCommand(
  ...
  if (member) {
    ...
    if (member.premiumSince) {
      embed.addFields({ name: '💎 Бустер с', value: `<t:${Math.floor(member.premiumSince.getTime() / 1000)}:R>`, inline: true });
    }
    ...
    if (roles) {
      embed.addFields({ name: `🎭 Роли (${member.roles.cache.size - 1})`, value: roles.slice(0, 1024) });
    }
  }
